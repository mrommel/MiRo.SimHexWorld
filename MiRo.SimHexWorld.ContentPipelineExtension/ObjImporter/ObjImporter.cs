...
namespace MiRo.SimHexWorld.ContentPipelineExtension.ObjImporter
{
            ...
            switch (lineTokens[0].ToLower())
            {
                ...
                case "f":
                    ...
                    for (int vertexIndex = 1; vertexIndex <= 3; vertexIndex++)
                    {
                        ...
                        if (positionMap == null)
                            throw new Exception("Positionmap not initalized!");

                        if (positionIndex >= positionMap.Length)
                            UpdatePositionMap();

                        if (positionIndex >= positionMap.Length )
                            throw new Exception(string.Format("Positionmap is to short [{0}-{1}] for {2})", 0, positionMap.Length, positionIndex));
